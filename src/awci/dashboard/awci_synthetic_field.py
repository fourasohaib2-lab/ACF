...
`awci.complexity.calculator.AWCICalculator` to produce a genuine AWCI grid for the
...
def awci_layer_grids(
    ...
    (`awci.hazards.updraft`/`awci.hazards.hydrometeor_phase`) - no new
    ...
    - "wind": real wind SPEED magnitude only - `_synthetic_inputs()`
      ...
      `awci.complexity.vertical_field` - a real vector wind layer for that
    ...
    - "ceiling" (added 2026-09-12, AWCI dashboard map panel's own
      "LAYERS" checkboxes): raw `awci.hazards.ceiling.
    ...
    - "visibility"/"dust" (added 2026-09-12): real
      `awci.hazards.visibility`/`awci.hazards.dust` [0, 1] risk proxies -
    ...
    `awci.hazards.hydrometeor_phase.compute_real_hydrometeor_phase_at_point()`'s
    own real severity), "convection" (m/s, real
    `awci.hazards.updraft.compute_real_max_updraft_velocity()` - a real,
    ...
    # loop - matches awci.complexity.spatial_field's own established reuse
...
def cross_section_phase_severity_field(
    ...
    fed into the real
    awci.hazards.hydrometeor_phase.compute_real_hydrometeor_phase_at_point()
