...
from awci.dashboard.awci_colors import AWCI_CMAP, LEVELS, level_for
from awci.dashboard.awci_synthetic_field import awci_grid, awci_layer_grids
...
class AWCIMapPanel(EventMixin, QWidget):
    ...
    def __init__(
        ...
        # awci.complexity.spatial_field.compute_real_complexity_field() result
        ...
        #: set_external_layer_grids() (awci.complexity.path_sampling.
    ...
    #: same real `awci.hazards.ceiling`/`awci.hazards.visibility`/`awci.hazards.dust`
    ...
    _EXTRA_LAYER_SPECS: dict[str, tuple[str, str, str]] = {
        ...
        "Turbulence": (
            ...
            "(TI2/EI), cited by ICAO Doc 9837 - awci.hazards.cat_turbulence - genuinely more rigorous, not "
            "just a proxy, though still distinct from a true EDR measurement (see that module's own "
            "docstring).",
        ),
        "Icing": (
            "icing", "PuBu",
            "Real awci.hazards.hydrometeor_phase surface precipitation-phase severity (Stull 2011 wet-bulb formula).",
        ),
        "Convection": (
            "convection", "Oranges",
            "Real awci.hazards.updraft maximum theoretical updraft velocity (m/s) - a real, disclosed nonlinear "
        ...
        "Ceiling": (
            "ceiling", "YlOrRd_r",
            "Real estimated ceiling height (m, awci.hazards.ceiling - LCL approximation). Reversed colormap: "
        ...
        "Visibility": (
            "visibility", "BuPu",
            "Real visibility-degradation risk proxy [0, 1] (awci.hazards.visibility - fog proximity; "
            "precipitation always 0.0, no real precipitation field in Real Physics mode). Real in both "
            "demo and Real Physics mode.",
        ),
        "Dust": (
            "dust", "YlOrBr",
            "Real dust/sand-storm emission-favorable-conditions risk proxy [0, 1] (awci.hazards.dust - wind "
    ...
    def set_external_layer_grids(self, layer_grids: dict[str, Any]) -> None:
        ...
        Visibility/Dust LAYERS data (awci.complexity.path_sampling.
