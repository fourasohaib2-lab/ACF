...
awci.complexity.calculator.AWCICalculator.calculate_module_scores() and
awci.complexity.normalizer.Normalizer - the real formula/range/blend weight
each module actually uses, not re-derived or guessed.

Honest real-vs-default disclosure: in "🔬 Real Physics" mode today,
awci.complexity.vertical_field.compute_real_complexity_volume() only supplies
...
from awci.complexity.wind_classification import classify_jet_stream, classify_wind_beaufort_force
...
#: awci.complexity.diagnostic_registry entry - only the 5 modules with a real
...
class ComponentInfo:
    ...
    description: str
    formula: str
    real_inputs: tuple[str, ...]  # real awci.complexity.calculator data-dict keys this module reads
...
COMPONENT_INFO: dict[str, ComponentInfo] = {
    ...
    "ceiling": ComponentInfo(
        ...
        description="Real estimated ceiling height (LCL approximation, awci.hazards.ceiling).",
        formula=(
    ...
    "visibility": ComponentInfo(
        ...
        description="Real visibility-degradation risk proxy (awci.hazards.visibility) - not a literal visibility distance.",
...
class AWCIComponentDetailDialog(QDialog):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        # - awci.complexity.diagnostic_registry.DIAGNOSTIC_REGISTRY existed
        ...
        # awci.complexity.result.build_awci_result()/AWCIResult.trace_chain(),
    ...
    def show_component(
        ...
        # awci.data.model_import); anything the file lacked fell back to
        ...
        if mode == "demo":
            ...
            is_real, source_label = True, "imported model file (awci.data.model_import)"
        else:
        ...
        if is_real:
            ...
            self.badge_label.setText(
                ...
                "(awci.complexity.vertical_field.compute_real_complexity_volume() does not supply this input yet)"
            )
        ...
        if diagnostic_key is not None:
            ...
            self.diagnostic_label.setText(
                "not yet in the centralized diagnostic registry (see awci.complexity.diagnostic_registry's own "
