...
class SoilHydrologyEngine:
    ...
    def soil_water_status(
        moisture_pct: float, field_capacity_pct: float = 30.0, wilting_point_pct: float = 12.0
    ) -> dict[str, Any]:
        ...
        if moisture_pct <= wilting_point_pct:
            ...
            available_water_pct = (
                (moisture_pct - wilting_point_pct) / (field_capacity_pct - wilting_point_pct)
            ) * 100.0
...
class GroundwaterEngine:
    ...
    @classmethod
    def evaluate_aquifer_recharge(
        cls, rainfall_mm: float, evapotranspiration_mm: float, soil_percolation_rate: float = 0.15
    ) -> float:
