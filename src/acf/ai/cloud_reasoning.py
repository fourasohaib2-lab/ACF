...
class CloudReasoningEngine:
    ...
    def explain_cumulonimbus_formation(
        ...
        if cape_j_kg > 1000:
            reasons.append(
                f"CAPE élevé ({cape_j_kg:.1f} J/kg > 1000 J/kg) procurant une forte énergie potentielle d'ascendance."
            )
            physical_mechanisms.append(
                "Poussée d'Archimède positive accélérant la parcelle d'air vers la tropopause (w_max = sqrt(2*CAPE))."
            )
        else:
        ...
        if surface_humidity_pct > 70:
            reasons.append(
                f"Humidité relative élevée en basse couche ({surface_humidity_pct:.0f}% > 70%), abaissant le LCL."
            )
            physical_mechanisms.append(
                "Condensation précoce entraînant la libération massive de chaleur latente de vaporisation (Lv)."
            )

        # 3. Convergence basse couche
        if low_level_convergence_s1 > 1e-5:
            reasons.append(
                "Convergence horizontale du vent en basse couche forçant l'ascendance mécanique au-dessus du LFC."
            )
        ...
        if shear_0_6km_m_s > 15:
            reasons.append(
                f"Fort cisaillement vertical du vent (0-6km: {shear_0_6km_m_s:.1f} m/s > 15 m/s) séparant le courant ascendant du courant descendant."
            )
            physical_mechanisms.append(
                "Organisation de la cellule en orage multicellulaire ou supercellulaire à longue durée de vie."
            )
        ...
        return {
            ...
            "verdict": "Conditions très favorables à la convection profonde"
            if (cape_j_kg > 1000 and surface_humidity_pct > 65)
            else "Conditions marginales",
