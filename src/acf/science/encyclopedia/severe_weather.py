...
def calculate_storm_relative_helicity(u_profile: List[float], v_profile: List[float], storm_u: float, storm_v: float, dz: float = 100.0) -> float:
...
def estimate_hail_size_mesh(mesh_mm: float) -> str:
    """Estimation empirique de la taille des grêlons en fonction de l'indice MESH (Maximum Expected Size of Hail)."""
    if mesh_mm < 20.0:
...
ENTRIES: List[EncyclopediaEntry] = [
    EncyclopediaEntry(
        ...
        variables={"V": "Profil du vent environnemental (m/s)", "c": "Vecteur déplacement de l'orage (m/s)", "omega_h": "Vorticité horizontale (s⁻¹)"},
        units={"SRH": "m²/s²"},
    ...
    EncyclopediaEntry(
        ...
        variables={"R": "Rayon du grêlon (m)", "LWC": "Liquid Water Content surfondue (kg/m³)", "Vh": "Vitesse de chute (m/s)"},
        units={"dM_h/dt": "kg/s"},
    ...
    EncyclopediaEntry(
        ...
        equation="MESH = 2.54 * (SHI)^0.5",
        latex_equation=r"\text{MESH} = 2.54 \times (\text{SHI})^{0.5} \quad [\text{mm}]",
        variables={"SHI": "Severe Hail Index (J/(m·s) intégré au-dessus de l'isotherme 0°C)"},
        units={"MESH": "mm"},
        description="Algorithme de télédétection radar estimant la taille maximale probable des grêlons atteignant le sol à partir de la réflectivité en altitude.",
        application_conditions=["Traitements de réflectivité radar 3D (NSSL MRMS, Météo-France MESH)"],
        limitations=["Effets de fonte sous la base du nuage"],
    ...
    EncyclopediaEntry(
        ...
        variables={"v_theta": "Vitesse tangentielles des vents (m/s)", "r": "Rayon du cœur du vortex (m)", "p": "Dépression centrale (hPa)"},
        ...
        application_conditions=["Mésocyclone de basse couche ou étirement de vorticité sur une frontière de méso-échelle"],
