...
def calculate_leighton_ozone_photoequilibrium(
    j_no2: float,
    no2_ppb: float,
    no_ppb: float,
    k_o3_no: float = 1.8e-14,
    air_number_density_molec_cm3: float = 2.46e19,
) -> float:
    """
    Calcul de la concentration d'ozone à l'état photostationnaire de Leighton
    [O3] = (j_NO2 * [NO2]) / (k_O3+NO * [NO]), retournée en ppb.

    NOTE (correction - erreur de plusieurs ordres de grandeur): k_o3_no
    (1.8e-14, vérifié correct pour la constante de vitesse bimoléculaire
    NO+O3->NO2+O2 à 298K, cf. JPL/littérature: 1.81e-14 +/- 0.13e-14
    cm^3 molecule^-1 s^-1) était appliqué DIRECTEMENT à des concentrations
    en ppb - alors qu'une constante bimoléculaire en cm^3 molecule^-1 s^-1
    exige des concentrations en molécules/cm^3. Sans conversion, la
    formule retournait un [O3] supérieur de ~11 ordres de grandeur à toute
    valeur atmosphérique physiquement possible (ex: j=0.008 s⁻¹,
    NO2=NO=5 ppb -> ancien résultat ~4.4e11 "ppb" au lieu de ~18 ppb,
    valeur réaliste). air_number_density_molec_cm3 convertit ppb <->
    molécules/cm³ (valeur par défaut ~298K/1 atm, WMO/US Standard
    Atmosphere) pour que le calcul reste physiquement cohérent en ppb.
    """
    if no_ppb <= 0.0 or k_o3_no <= 0.0 or air_number_density_molec_cm3 <= 0.0:
        return 0.0
    k_pseudo_first_order_ppb_s = k_o3_no * air_number_density_molec_cm3 * 1e-9
    return (j_no2 * no2_ppb) / (k_pseudo_first_order_ppb_s * no_ppb)
...
ENTRIES: list[EncyclopediaEntry] = [
    EncyclopediaEntry(
        ...
        variables={
            "j_NO2": "Taux de photolyse du NO2 (s⁻¹)",
            "k_O3+NO": "Constante de réaction NO + O3 -> NO2 + O2",
            "[NO2], [NO]": "Concentrations des oxydes d'azote",
        },
        units={"[O3]": "ppb"},
        ...
        limitations=[
            "Violée en présence de radicaux péroxydes (RO2/HO2) issus de la dégradation des COV (formation de smog)"
        ],
        references=[
            "Leighton (1961) Photochemistry of Air Pollution",
            "Seinfeld & Pandis (2016) Atmospheric Chemistry",
        ],
    ...
    EncyclopediaEntry(
        ...
        limitations=[
            "Les limites de concentration recommandées par l'OMS sont de 5 µg/m³ en moyenne annuelle pour les PM2.5"
        ],
    ...
    EncyclopediaEntry(
        ...
        variables={
            "vd": "Vitesse de dépôt sec (m/s)",
            "Ra": "Résistance aérodynamique",
            "Rb": "Résistance de couche limite quasi-laminaire",
            "Rc": "Résistance stomatique/surface",
            "Lambda": "Coefficient de lessivage par la pluie",
        },
