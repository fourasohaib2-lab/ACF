...
class ESOCController:
    ...
    def handle_refresh_observations(self) -> dict[str, Any]:
        ...
        Genuinely triggers a real, async refresh of all - reusing the
        ...
        EarthMonitoringPanel - PIREP (real NOAA pilot reports, not
        AMDAR, which has no free public feed ACF can reach). Updated
        here too, in the same commit that adds it, specifically to not
        repeat the staleness this NOTE itself documents.
        """
        ...
        self.dispatcher.run_async(_PIREPFetchWorker(PIREPConnector()).run)
