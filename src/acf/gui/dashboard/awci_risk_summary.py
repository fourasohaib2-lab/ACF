...
Small badge list (Turbulence / Icing / Convective / Overall) matching the
reference mockup's "RISK SUMMARY" panel. Levels are derived from the real
AWCICalculator module scores for the route's worst point, not invented
independently of the AWCI computation.
"""
...
class AWCIRiskSummary(QWidget):
    ...
    def update_data(self, module_scores: dict[str, float], overall_awci: float) -> None:
        for key, _icon, _label, module in _ROWS:
            ...
            score = overall_awci if module is None else module_scores.get(module, 0.0)
            level = _band(score)
