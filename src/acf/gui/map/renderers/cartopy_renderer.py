...
class CartopyRenderer(BaseRenderer):
    ...
    def render(self, ax: Any, data: Any | None = None, **kwargs) -> Any:
        ...
        if kwargs.get("ocean", False):
            ax.add_feature(cfeature.OCEAN, color=kwargs.get("ocean_color", "lightblue"), alpha=self.opacity * 0.3)

        # Land
        if kwargs.get("land", False):
            ax.add_feature(cfeature.LAND, color=kwargs.get("land_color", "lightgreen"), alpha=self.opacity * 0.2)
