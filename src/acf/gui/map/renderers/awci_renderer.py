...
class AWCIRenderer(BaseRenderer):
    ...
    def _extract_arrays(self, data: dict):
        ...
        if lons is None:
            raise ValueError(
                "Longitude not found. Available keys: " + ", ".join(data.keys()) +
                ". Expected one of: 'longitude', 'lon', 'lons', 'lng'"
            )
        if lats is None:
            raise ValueError(
                "Latitude not found. Available keys: " + ", ".join(data.keys()) +
                ". Expected one of: 'latitude', 'lat', 'lats'"
            )
        
        return awci, lons, lats
    
    def render(self, ax: Any, data: Optional[Union[Dict, Any]] = None, **kwargs) -> Any:
