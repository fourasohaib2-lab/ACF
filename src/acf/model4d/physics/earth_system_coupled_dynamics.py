...
class EarthSystemCoupledDynamics:
    ...
    def calculate_energy_balance(self, state: EarthSystemState) -> float:
        """
        Calculate simplified planetary energy balance.
        """

        absorbed = state.atmospheric_energy + state.ocean_energy + state.greenhouse_forcing

        losses = state.ice_fraction * 10 + state.biosphere_activity

        return absorbed - losses

    def simulate(self, state: EarthSystemState, timestep: float = 1.0):
        """
        ...
        new_atmosphere = state.atmospheric_energy + balance * 0.1 * timestep

        new_ocean = state.ocean_energy + balance * 0.05 * timestep

        new_ice = max(0.0, min(1.0, state.ice_fraction - balance * 0.001))
    ...
    def climate_feedback_index(self, state: EarthSystemState) -> float:
        """
        Estimate coupled climate feedback strength.
        """

        return state.greenhouse_forcing * (1 - state.ice_fraction) * state.biosphere_activity
