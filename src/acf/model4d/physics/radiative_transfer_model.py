...
class RadiativeTransferModelPhysics:
    ...
    @staticmethod
    def transmitted_radiation(incoming_flux: float, absorptivity: float) -> float:
        """
    ...
    @staticmethod
    def emitted_radiation(temperature: float, emissivity: float = 1.0) -> float:
        """
        ...
        return round(emissivity * RadiativeTransferModelPhysics.STEFAN_BOLTZMANN * temperature**4, 10)

    @staticmethod
    def optical_depth(absorption_coefficient: float, path_length: float) -> float:
        """
        ...
        return round(absorption_coefficient * path_length, 10)

    @staticmethod
    def transmission_from_optical_depth(optical_depth: float) -> float:
        """
        ...
        return round(math.exp(-optical_depth), 10)

    @staticmethod
    def radiative_balance(absorbed: float, emitted: float) -> float:
        """
        Net radiative balance.

        Balance = absorbed - emitted
        """
        return round(absorbed - emitted, 10)

    @staticmethod
    def greenhouse_effect(surface_temperature: float, atmospheric_emissivity: float) -> float:
        """
        ...
        return round(
            RadiativeTransferModelPhysics.STEFAN_BOLTZMANN * surface_temperature**4 * (1 - atmospheric_emissivity), 10
        )
