...
class AtmosphericRadiationBalanceDynamics:
    ...
    def solar_absorption(self, state: RadiationState) -> float:
        ...
        return round(state.solar_radiation * state.atmospheric_absorption / 10, 2)
    ...
    def infrared_emission(self, state: RadiationState) -> float:
        ...
        return round(state.infrared_radiation / 20, 2)
    ...
    def greenhouse_effect(self, state: RadiationState) -> float:
        ...
        return round(state.greenhouse_gas / 2, 2)
    ...
    def cloud_radiative_feedback(self, state: RadiationState) -> float:
        ...
        return round(state.cloud_fraction * 5, 2)
    ...
    def radiative_equilibrium(self, state: RadiationState) -> float:
        ...
        equilibrium = (
            self.solar_absorption(state) + self.greenhouse_effect(state) - self.cloud_radiative_feedback(state) + 1.25
        )
    ...
    def outgoing_longwave_radiation(self, state: RadiationState) -> float:
        ...
        return round(state.temperature * 0.05 + state.infrared_radiation * 0.1, 2)
    ...
    def atmospheric_energy_balance(self, state: RadiationState) -> float:
        ...
        balance = self.solar_absorption(state) + self.greenhouse_effect(state) - self.outgoing_longwave_radiation(state)
