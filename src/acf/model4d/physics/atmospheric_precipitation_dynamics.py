...
class AtmosphericPrecipitationDynamics:
    ...
    def condensation_amount(self, state: PrecipitationState) -> float:
        ...
        result = state.humidity * state.condensation_rate * 0.1
    ...
    def precipitation_efficiency(self, state: PrecipitationState) -> float:
        """
        Rendement conversion nuage → pluie.
        """

        result = state.condensation_rate * state.convection_intensity * state.precipitation_efficiency
    ...
    def water_state(self, state: PrecipitationState) -> float:
        ...
        result = state.humidity * state.condensation_rate

        return round(result, 2)

    def cloud_conversion(self, state: PrecipitationState) -> float:
        """
        Conversion eau nuageuse vers hydrométéores.
        """

        result = state.humidity * state.precipitation_efficiency * 0.05
