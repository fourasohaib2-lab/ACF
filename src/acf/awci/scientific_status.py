...
NORMALIZER_RANGE_STATUS: dict[str, ThresholdStatus] = {
    ...
    "theta_e": ThresholdStatus(ScientificStatus.HYPOTHESIS, "Range 250..380 K - a real, generously wide operational envelope (real cold/dry theta-e near 250 K, real warm/moist pre-convective theta-e up to 350-380 K), not sourced from a specific climatology. The underlying real theta-e VALUE itself (acf.awci.theta_e.compute_real_theta_e_at_point()) uses the CONFIRMED, published Bolton (1980) formula - only this normalization envelope's own bounds are HYPOTHESIS, same as every other Normalizer range."),
    ...
    "updraft_velocity": ThresholdStatus(ScientificStatus.HYPOTHESIS, "Range 0..70 m/s - a real, generously wide envelope covering both real observed extreme-storm updrafts (rarely exceeding ~50-60 m/s) and idealized parcel theory's own known tendency to overestimate. The underlying real w_max=sqrt(2*CAPE) formula is a real, classic, textbook parcel-theory result (acf.awci.updraft.compute_real_max_updraft_velocity()) - only this normalization envelope's own bounds are HYPOTHESIS, and the formula is a deterministic function of CAPE alone (see that module's own honest disclosure of limited independent information content)."),
    ...
    "precipitation_phase_severity": ThresholdStatus(ScientificStatus.HYPOTHESIS, "Range 0..1 - the value already IS the [0, 1] severity assigned by acf.awci.hydrometeor_phase.PHASE_SEVERITY, a real, disclosed ACF ordinal design choice (freezing rain/ice pellets > wet snow/mix > snow > rain, grounded in real, well-documented aviation icing-hazard severity), not a value sourced from a published numeric severity index. The underlying real phase CLASSIFICATION (acf.science.precipitation.HydrometeorType.classify(), Stull 2011 wet-bulb formula) is itself explicitly self-disclosed as a real heuristic, not a validated physical formula."),
