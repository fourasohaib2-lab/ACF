...
interface Row { key: string; label: string; unit: string; digits: number; convert?: (v: number) => number; nullText?: string }
...
const SURFACE_ROWS: Row[] = [
  ...
  { key: "ceiling_m", label: "Plafond (OACI)", unit: "ft", digits: 0, convert: (v) => v / 0.3048, nullText: "pas de plafond" },
...
export function Inspector({ point, registry }: Props) {
  ...
  const row = (r: Row, values: Record<string, number | null>) => {
    if (!(r.key in values)) return null;
    const v = values[r.key];
    return (
      <tr key={r.key}>
        <th scope="row">{r.label}</th>
        <td className="num">{v === null || v === undefined ? (r.nullText ?? "—") : fmt(r.convert ? r.convert(v) : v, r.digits, r.unit)}</td>
