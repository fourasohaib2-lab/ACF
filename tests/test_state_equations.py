...
def test_pressure():
    value = StateEquations.pressure(1.2, 300)

    assert round(value, 2) == 103338.0


def test_density():
    value = StateEquations.density(101325, 288)

    assert round(value, 2) == 1.23


def test_virtual_temperature():
    value = StateEquations.virtual_temperature(300, 0.01)
...
def test_zero_temperature_density():
    with pytest.raises(ValueError):
        StateEquations.density(100000, 0)
