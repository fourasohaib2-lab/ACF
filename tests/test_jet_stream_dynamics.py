from acf.model4d.physics.jet_stream_dynamics import JetStreamDynamics, JetStreamState


def create_model():

    state = JetStreamState(wind_speed=60.0, temperature_gradient=0.8, vertical_shear=0.4, latitude=45.0)
...
def test_high_wind():
    ...
    state = JetStreamState(100, 1, 0.5, 50)
...
def test_equator_coriolis():
    ...
    state = JetStreamState(40, 0.5, 0.2, 0)
...
def test_negative_gradient():
    ...
    state = JetStreamState(40, -0.5, 0.3, 40)

    model = JetStreamDynamics(state)

    assert isinstance(model.thermal_wind_balance(), float)
