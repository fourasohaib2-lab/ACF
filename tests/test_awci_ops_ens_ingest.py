...
class DropMember(FixtureFetcher):
    ...
    def __init__(self, member: int) -> None:
        ...
        for step in (0, 6):
            index = (ENS_FIXTURE / f"20260925000000-{step}h-enfo-ef.index").read_text().splitlines()
...
def test_a_failing_member_is_excluded_and_recorded(tmp_path: Path) -> None:
    ...
    ds = EnsStore(tmp_path).dataset("fixture", "2026092500")
...
def test_a_missing_step_is_reported_and_never_filled(tmp_path: Path) -> None:
    ...
    ds: xr.Dataset = EnsStore(tmp_path).dataset("fixture", "2026092500")
...
def test_command_line(tmp_path: Path, monkeypatch) -> None:
    ...
    assert main(["--run", "2026092500", "--domain", "fixture", "--steps", "0,6", "--members", "1-2",
                 "--domains-file", str(domains), "--data-dir", str(tmp_path)]) == 0
    assert EnsStore(tmp_path).manifest("fixture", "2026092500")["members_requested"] == [1, 2]
