...
def test_set_indices_shows_the_real_lifted_and_showalter_indices(qapp):
    widget = ACFStabilityIndicesWidget()
    volume = compute_real_complexity_volume(model="ALADIN", n_lat=10, n_lon=18, n_levels=8, steps=3, perturbation_scale=2.0, seed=1)
    indices = compute_real_stability_indices_at_point(volume, lat=10.0, lon=20.0)
    ...
    assert f"{indices['lifted_index']:.1f}" in widget._labels["Lifted Index"].text()
    assert indices["lifted_index_category"] in widget._labels["Lifted Index"].text()
    assert f"{indices['showalter_index']:.1f}" in widget._labels["Showalter Index"].text()
