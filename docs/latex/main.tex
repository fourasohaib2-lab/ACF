...
% Added during a hygiene cleanup pass, per ROADMAP.md's "reconcile
% certificate/sprint-report documents" near-term priority extended to
% docs/ subdirectories. This ~250-file LaTeX source was not touched
% chapter by chapter - only this one disclaimer was added, at the
% front of the compiled document, where a reader will actually see it.
\begin{quote}
\textcolor{red}{\textbf{Historical / unverified document.}} This document's specific
statistics, capability claims, and hardware figures (test counts, "100\% Pass
Rate", GPU/MPI cluster figures, AI acceleration factors, etc.) were not
independently reproduced and in several places are known to be inaccurate
(e.g. the "1000x" AI surrogate acceleration claim below was found fabricated
and removed from the real code during this project's 2026 audit - see
\texttt{docs/ACF\_PHYSICS\_GUARD\_AUDIT\_CHANGELOG.md}). For the project's
actual, reproducible test/status numbers, see \texttt{ROADMAP.md} and
\texttt{README.md}'s "Verified Status" section in the repository root. Treat
any specific number or "CERTIFIED"/"COMPLETE" claim in the chapters below as
aspirational unless it also appears in one of those two documents.
\end{quote}
...
\begin{itemize}
    \item \textbf{Total Python Modules:} 1301
    \item \textbf{Total Object-Oriented Classes:} 1285
    \item \textbf{Total Callable Functions \& Methods:} 3883
    \item \textbf{Total Source Lines of Code (LOC):} 79029
    \item \textbf{Registered Scientific Packages:} 503
...
\midrule
Total Python Packages & 503 \\
Total Python Modules & 1301 \\
Total Classes & 1285 \\
Total Functions/Methods & 3883 \\
Total Lines of Code (LOC) & 79029 \\
