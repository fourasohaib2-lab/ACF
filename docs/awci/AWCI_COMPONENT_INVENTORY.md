...
| 12 | LAYERS panel | `awci_map_panel.py::_build_layers_panel` | 🟡 | All 7 toggles real — Wind/Turbulence/Icing work in BOTH demo (`awci_synthetic_field.awci_layer_grids()`) and Real Physics mode (`acf.awci.path_sampling.real_layer_grids_at_level()`, built 2026-09-03); Convection/CAPE/Clouds are demo mode only — the real solver volume carries no CAPE/precipitation field, a disclosed scope limit, real no-op there — see AWCI_BUTTON_CONTRACT.md |
