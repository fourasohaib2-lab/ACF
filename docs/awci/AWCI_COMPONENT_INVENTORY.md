...
| 27 | 🔍 See Vertical Profile button + dialog | `awci_vertical_profile.py::AWCIVerticalProfile` | ✅ | real composite AWCI score per level (§51's full real level list, demo mode) — every bar is now clickable, opening a real 9-module breakdown dialog (`AWCIVerticalProfileLevelDialog`); "stabilité"/"turbulence" honestly disclosed as having no dedicated module, not fabricated |
