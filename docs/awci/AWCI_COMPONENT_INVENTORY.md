...
| 12 | LAYERS panel | `awci_map_panel.py::_build_layers_panel` | 🟡 | All 10 toggles real — Wind/Turbulence/Icing/Ceiling/Visibility/Dust work in BOTH demo (`awci_synthetic_field.awci_layer_grids()`) and Real Physics mode (`acf.awci.path_sampling.real_layer_grids_at_level()`; Wind/Turbulence/Icing built 2026-09-03, Ceiling/Visibility/Dust added 2026-09-12); Convection/CAPE/Clouds are demo mode only — the real solver volume carries no CAPE/precipitation field, a disclosed scope limit, real no-op there — see AWCI_BUTTON_CONTRACT.md |
...
| 27 | 🔍 See Vertical Profile button + dialog | `awci_vertical_profile.py::AWCIVerticalProfile` | ✅ | real composite AWCI score per level (§51's full real level list) — every bar is now clickable, opening a real 9-module breakdown dialog (`AWCIVerticalProfileLevelDialog`); "stabilité"/"turbulence" honestly disclosed as having no dedicated module, not fabricated. Real in BOTH modes since 2026-09-04: demo mode's own `_synthetic_inputs()`, Real Physics mode via real log-pressure interpolation between the volume's native levels (`acf.awci.vertical_field.vertical_profile_at_standard_levels()`) — a level outside a given point's real native vertical range is honestly omitted, never extrapolated |
...
| 43 | 📡 Real Archive button + dialog | `awci_dashboard.py` + `acf/awci/archive_field.py` (new) | ⬜→✅ 2026-09-04 | a genuine THIRD data tier: real archived ALADIN 00Z FULLPOS output (2026-08-31, North Africa), decoded via EPyGrAM straight from its real FA file — see `docs/awci/AWCI_REAL_ARCHIVE_DATA.md` for the full honest scope/limits |
| 44 | LAYERS: Ceiling/Visibility/Dust checkboxes | `awci_map_panel.py` + `acf/awci/ceiling.py`/`visibility.py`/`dust.py` | ⬜→✅ 2026-09-12 | real in BOTH demo (`awci_layer_grids()`) and Real Physics mode (`real_layer_grids_at_level()`) — unlike #41, these 3 only need temperature/specific humidity/pressure/wind speed, all real and already available in either mode. Ceiling uses a reversed colormap (`"YlOrRd_r"`) since a lower value is more hazardous, unlike every other layer here |
